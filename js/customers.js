        ...
        try {
            ...
            if (!salesSnapshot.empty) {
                salesSnapshot.forEach((docSnap) => {
                    ...
                    const mobile = sale.mobile || sale.phone || sale.customerPhone || sale.customerMobile || "N/A";
            ...
            console.warn("Firestore customer fetch failed, checking LocalStorage fallback:", e);
        }

        // Fetch LocalStorage Records
        const localCustomers = JSON.parse(localStorage.getItem('customers')) || [];
        localCustomers.forEach(localCust => {
            const mob = localCust.mobile || "N/A";
...
if (resetCustBtn) {
    resetCustBtn.addEventListener('click', function () {
        const isConfirmed = window.confirm("⚠️ Are you sure you want to permanently reset all customer records and local bill history?");
        ...
        if (isConfirmed) {
            ...
            alert("Customer history reset successfully!");
            location.reload();
        }
    });
}
...
// 7. Global Modal Handler for Detailed Invoices
window.viewCustomerBills = function(mobile) {
    ...
    if (sortedBills.length === 0) {
        billsListHtml = `<p style="text-align:center; padding:15px; color:#666;">No recorded item details for this customer.</p>`;
    } else {
